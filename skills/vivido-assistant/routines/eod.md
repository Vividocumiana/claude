...
2. `bash ~/.claude/skills/vivido-assistant/send.sh U062MREADAB /tmp/vivido-assistant-eod.md` (MAI `D0634QNLF52`, il bot non la vede → `channel_not_found`).
3. Fallback retry 8s → `send.sh U062MREADAB` (stessa DM, indirizzata via user ID).
4. Rispondi: `✅ EOD inviato (POV:<KL_status> · <P> progetti · 🔴<R> 🟡<Y> 🟢<G> · 🤝<Partner> · CRM:<N> · 💰Fatture:<F> · 📜Contratti:<C> · 🪦<S> · 💡<Sp> spunti · 👥<persone> · ❓<Q> domande).`

L'EOD viene consegnato nella DM bot ↔ Samuele (`U062MREADAB`). **Il founder risponde in thread sulla DM stessa** — niente più reply su #company-brain. La routine `log-ingest` alle 22:00 cerca in quella DM il messaggio più recente del bot che inizia con `🌙 *EOD Debrief` di oggi e legge le sue reply.
