...
- `channel_id` di default = `<VIVIDO_DM_CHANNEL>` (DM bot Vivido ↔ founder). Fallback: `<VIVIDO_FOUNDER_SLACK>` (user ID founder; `chat.postMessage` lo risolve sulla stessa DM).
...
2. Invia con `send.sh <VIVIDO_DM_CHANNEL>`.
...
5. Invia con `send.sh` alla DM `<VIVIDO_DM_CHANNEL>`.
